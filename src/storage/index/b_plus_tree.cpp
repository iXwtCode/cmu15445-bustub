...
auto BPLUSTREE_TYPE::GetValue(const KeyType &key, std::vector<ValueType> *result, Transaction *txn) -> bool {
  ...
  while (true) {
    ...
    root_page = root_page_guard.template As<BPlusTreePage>();
...
auto BPLUSTREE_TYPE::SplitLeaf(LeafPage *leaf, const KeyType &key, const ValueType &value, page_id_t *new_id)
  ...
  auto new_leaf = new_leaf_basic_guard.template AsMut<LeafPage>();
  new_leaf_basic_guard.SetDirty(true);
  new_leaf_basic_guard.Drop();

  auto new_leaf_guard = bpm_->FetchPageWrite(*new_id);
  new_leaf = new_leaf_guard.template AsMut<LeafPage>();
...
auto BPLUSTREE_TYPE::SplitInternal(InternalPage *internal, const KeyType &key, page_id_t *new_id,
  ...
  auto new_internal = new_internal_basic_guard.template AsMut<InternalPage>();
  ...
  new_internal = new_internal_guard.template AsMut<InternalPage>();
...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value, Transaction *txn) -> bool {
  ...
  auto header_page = ctx.header_page_guard_->template AsMut<BPlusTreeHeaderPage>();

  if (header_page->root_page_id_ == INVALID_PAGE_ID) {
    BasicPageGuard root_page_basic_guard = bpm_->NewPageGuarded(&header_page->root_page_id_);
    auto root_page_basic = root_page_basic_guard.template AsMut<LeafPage>();
  ...
  while (true) {
    if (root_page->IsLeafPage()) {
      ...
      if (leaf->GetSize() == leaf->GetMaxSize()) {
        ...
        while (!ctx.write_set_.empty()) {
          ...
          auto internal_parent = internal_parent_guard.template AsMut<InternalPage>();
        ...
        if (!split_fin) {
          ...
          auto new_root_page = new_root_page_basic_guard.template AsMut<InternalPage>();
          ...
          new_root_page = new_root_page_guard.template AsMut<InternalPage>();
    ...
    root_page = root_page_guard.template AsMut<BPlusTreePage>();
...
void BPLUSTREE_TYPE::RemoveResGuardsPop(std::deque<WritePageGuard> &guards, std::deque<int> &keys_index,
  ...
  while (!guards.empty()) {
    ...
    guards.pop_back();
    auto parent_internal = parent_guard.template AsMut<InternalPage>();
  ...
  while (true) {
    if (root_page->IsLeafPage()) {
      ...
      while (!ctx.write_set_.empty() && cur_internal->GetSize() - 1 < cur_internal->GetMinSize()) {
        ...
        parent_internal = parent_guard.template AsMut<InternalPage>();
